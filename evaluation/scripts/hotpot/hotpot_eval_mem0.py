...
def mem0_search(user_id: str, query: str, top_k):
    results = None
    for attempt in range(max_retries):
        try:
    ...
    texts, sps = extract_texts_and_sps_from_mem0_memory(memories)
    for t, s in sps:
        sp_list.append([t, s])
...
def extract_texts_and_sps_from_mem0_memory(memories: list[dict]):
    texts: list[str] = []
...
def build_and_ask(item):
    ...
    try:
        context, sp_list = mem0_search(qid, question, top_k=7)
...
pred_path = os.path.join(output_dir, "dev_distractor_pred_mem0.json")
