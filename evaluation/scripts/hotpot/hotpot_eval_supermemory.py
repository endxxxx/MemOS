...
load_dotenv()
os.environ["SEARCH_MODE"] = os.environ.get("SEARCH_MODE", "fast")
...
def supermemory_search(user_id: str, query: str, top_k):
    results = None
    for attempt in range(max_retries):
        try:
    ...
    memories = [r.metadata["raw"] for r in results.results]
    context = MEMOS_CONTEXT_TEMPLATE.format(user_id=user_id, memories="\n".join(memories))

    # Extract supporting facts (sp) from raw sources
    sp_list: list[list[str | int]] = []
    texts, sps = extract_texts_and_sp_from_sources(memories)
    for t, s in sps:
        sp_list.append([t, s])
...
def extract_texts_and_sps_from_supermemory(memories: list[str]):
    texts: list[str] = []
...
def build_and_ask(item):
    ...
    try:
        context, sp_list = supermemory_search(qid, question, top_k=7)
...
pred_path = os.path.join(output_dir, "dev_distractor_pred_supermemory.json")
